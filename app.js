function log() {
    ...
    .then(response => response.json())
        .then(json => {
            for (let index = 0; index < json.length; index++) {
                document.getElementById("suser").insertAdjacentHTML("beforeend","<option value="+json[index].id+" onclick=loguser() id="+json[index].name+">"+json[index].name+"</option>")
            }
    })
    ...
    function caruser(){
        ...
        let contenido= ""
        let f = document.getElementById("suser")
